...
let shareCodes = [ // 这个列表填入你要助力的好友的shareCode
    '98cc7fdc892b4a0dabdc244f884a97cd'
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://119.29.240.238/jd/shareCodes.php?shareCodeType=JD_SHARES_FRUIT&shareCodesNum=20`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          ...
          if (data) {
            console.log(`随机取个20码放到您固定的互助码后面(不影响已有固定互助)`)
