...
!(async () => {
    ...
    if (flag_hb) {
        ...
        $.inviteCodeList_rp = [...($.shareCode || []), ...($.inviteCodeList_rp || [])]
    ...
    $.inviteCodeList = [...($.shareCode || []), ...($.inviteCodeList || [])]
    for (let j = 0; j < cookiesArr.length; j++) {
        $.cookie = cookiesArr[j];
        $.UserName = decodeURIComponent($.cookie.match(/pt_pin=(.+?);/) && $.cookie.match(/pt_pin=(.+?);/)[1]);
        token = await getJxToken();
...
function getShareCode(name) {
    return new Promise(resolve => {
        $.get({
            url: "http://119.29.240.238/jd/shareCodes.php?shareCodeType=JD_SHARES_JXMC&shareCodesNum=10&type="+name,
            headers: {
            ...
            try {
                if (err) {
                    ...
                    console.log(`优先账号内部互助，有剩余助力次数再帮【cc】助力`);
